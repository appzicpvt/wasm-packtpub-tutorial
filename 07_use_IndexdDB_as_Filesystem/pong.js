...
function showName() {
    ...
    eleName.innerText = gameState.name;
    document.getElementsByTagName("body")[0].appendChild(eleName);
}

function handleBtnSubmit() {

    FS.mkdir("/playerdb");
    FS.mount(IDBFS, {}, '/playerdb');
    FS.syncfs(true, function () {
        setupGameState();
        document.getElementById("inuptName").remove();
        document.getElementById("btnSubmit").remove();
        render();
        showName();
        console.log("readFile", FS.readFile("/player-data/a"));
    })
}

function setupGameState() {
    MOVE = Module.Move
    const name = document.getElementById("inuptName").value;
