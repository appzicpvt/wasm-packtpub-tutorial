...
std::pair<int, int> getScore(const std::string &name)
{
    const std::string fileName("/playerdb/player/" + name);
...
{
    auto [leftScore, rightScore] = getScore(name);
    drawCanvas(800, 600);
...
{
    if (!std::filesystem::exists("/playerdb/player/"))
    {
        std::filesystem::create_directory("/playerdb/player/");
    }
    const auto fileName = "/playerdb/player/" + gameState.name;
